...
class HybridAutoencoder(keras.Model):
    ...
    def call(self, x, training=None):
        ...
        predicted_loss = self.lpm(encoded, training=training)
...
class CustomTrainingLoop:
    ...
    def __init__(self, model, learning_rate=0.001, optimizer=None,
                 reconstruction_weight=1.0, lmp_weight=0.1, **kwargs):
        """
        Initialize custom training loop

        Args:
            model: HybridAutoencoder model
            learning_rate: Learning rate for optimizer (used if optimizer is None)
            optimizer: Custom optimizer (if None, creates Adam with learning_rate)
            reconstruction_weight: Weight for reconstruction loss
            lmp_weight: Weight for LPM loss
